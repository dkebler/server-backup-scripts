...
create_EBS_Snapshot_Tags() {
  ...
  if $name_tag_create; then
    snapshot_tags="$snapshot_tags Key=Name,Value=$current_date_human-HW_Server_Root-${ebs_selected}"
  fi
  ...
  if $user_tags; then
    snapshot_tags="$snapshot_tags Key=Volume,Value=${ebs_selected} Key=Created,Value=$current_date_human"
  fi
...
current_date_human=$(date +%a-%b-%d-%Y--%H:%M)
...
if [[ -n $purge_after_input ]]; then
  ...
  echo "Snapshots taken by $app_name will be eligible for purging after the following date : $(date -d @$purge_after_date_fe)"
fi
...
for ebs_selected in $ebs_backup_list; do
 ec2_snapshot_description="Automated backup of EBS ${ebs_selected} on $(date -d @$current_date) purge after $(date -d @$purge_after_date_fe)"
